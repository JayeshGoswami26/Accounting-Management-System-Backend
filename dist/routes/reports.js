...
router.get("/outstanding-by-customer", async (req, res) => {
    const bookings = await Booking_1.Booking.find({ remainingBalance: { $gt: 0 } }).populate("customerId");
...
router.get("/overdue", async (req, res) => {
    ...
    const items = await Booking_1.Booking.find({
        travelDate: { $lt: today },
...
router.get("/profit-loss", async (req, res) => {
    ...
    const bookings = await Booking_1.Booking.find(match).populate("customerId");
    const totalProfit = bookings.reduce((sum, b) => sum + (b.profit || 0), 0);
    const totalSold = bookings.reduce((sum, b) => sum + (b.soldAt || 0), 0);
    const totalCost = bookings.reduce((sum, b) => sum + (b.trueCost || 0), 0);
    const customerMap = new Map();
    bookings.forEach((b) => {
        const customerId = b.customerId?._id?.toString() || b.customerId?.toString();
        if (!customerMap.has(customerId)) {
...
router.get("/loss-bookings", async (req, res) => {
    const bookings = await Booking_1.Booking.find({ profit: { $lt: 0 } })
        .populate("customerId")
        .sort({ travelDate: -1 });
    const customerMap = new Map();
    bookings.forEach((b) => {
...
router.get("/channel-totals", async (req, res) => {
    ...
    const payments = await Payment_1.Payment.find(match);
    const totals = {};
    payments.forEach((p) => {
