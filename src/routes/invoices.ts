...
import { authenticate, canViewInternalData, AuthRequest } from "../middleware/auth";
...
router.get("/:id", optionalAuth, async (req: any, res) => {
  const invoice = await Invoice.findById(req.params.id).populate("customerId bookingIds createdBy updatedBy");
  if (!invoice) {
...
router.post("/from-booking/:bookingId", optionalAuth, async (req: any, res) => {
  const booking = await Booking.findById(req.params.bookingId).populate("customerId");
  if (!booking) {
    return res.status(404).json({ message: "Booking not found" });
  }
  const existingInvoice = await Invoice.findOne({ bookingIds: booking._id });
  if (existingInvoice) {
  ...
  await invoice.save();
  const populated = await Invoice.findById(invoice._id).populate("customerId bookingIds createdBy");
...
router.put("/:id", optionalAuth, async (req: any, res) => {
  ...
  const bookings = await Booking.find({ _id: { $in: invoice.bookingIds } }).populate("customerId");
  ...
  invoice.updatedBy = req.user?._id ? new mongoose.Types.ObjectId(req.user._id) : null;
  await invoice.save();
  const populated = await Invoice.findById(invoice._id).populate("customerId bookingIds createdBy updatedBy");
  res.json(populated);
});

router.get("/:id/export/csv", async (req, res) => {
  const invoice = await Invoice.findById(req.params.id).populate("customerId bookingIds");
  if (!invoice) {
    return res.status(404).json({ message: "Invoice not found" });
  }
  let settings = await Settings.findOne();
  ...
  const bookings: any[] = inv.bookingIds || [];
  
  let bookingDetailsRows: string[][] = [];
...
router.get("/export/csv", async (req, res) => {
  const invoices = await Invoice.find().sort({ invoiceDate: -1 }).populate("customerId bookingIds");
  const headers = [
    "Invoice Number",
    "Invoice Date",
    "Customer Name",
...
router.get("/:id/export/docx", async (req, res) => {
  try {
    ...
    const inv: any = invoice;
    const bookingIds: any[] = inv.bookingIds || [];
    const booking: any = bookingIds[0] || {};
